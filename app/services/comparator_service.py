    ...
    def compare_versions(new_version_id, old_version_id=None):
        try:
            ...
            if not old_version_id:
                comparison = Comparison(document_version_id=new_version_id, compare_version_id=None, differences_count=0, differences_data={'differences': []})
                db.session.add(comparison)
                db.session.commit()
                return {'differences_count': 0, 'differences': [], 'comparison_id': comparison.id}
            ...
            for sheet_name in all_sheets:
                    ...
                    for r_idx, row in enumerate(new_rows):
                        for c_idx, cell in enumerate(row):
                            if cell is not None and cell != '':
                                differences.append({'sheet': sheet_name, 'row': r_idx + 1, 'col': get_column_letter(c_idx + 1), 'old_value': '', 'new_value': str(cell)})
                    continue
                if sheet_name not in new_sheets:
                    ...
                    for r_idx, row in enumerate(old_rows):
                        for c_idx, cell in enumerate(row):
                            if cell is not None and cell != '':
                                differences.append({'sheet': sheet_name, 'row': r_idx + 1, 'col': get_column_letter(c_idx + 1), 'old_value': str(cell), 'new_value': ''})
                    continue
                new_rows = extracted_data[sheet_name]
                old_rows = old_data[sheet_name]
                    ...
                    for col_idx in range(max_cols):
                        ...
                        if new_val != old_val:
                            if (new_val is None or new_val == '') and (old_val is None or old_val == ''):
                                continue
                            differences.append({'sheet': sheet_name, 'row': row_idx + 1, 'col': get_column_letter(col_idx + 1), 'old_value': str(old_val) if old_val is not None else '', 'new_value': str(new_val) if new_val is not None else ''})
            comparison = Comparison(document_version_id=new_version_id, compare_version_id=old_version_id, differences_count=len(differences), differences_data={'differences': differences, 'new_sheets': list(new_sheets), 'old_sheets': list(old_sheets)})
            db.session.add(comparison)
            db.session.commit()
            return {'differences_count': len(differences), 'differences': differences, 'comparison_id': comparison.id}
