"""Serviço de Documentos - Upload, versionamento, extração de dados e aprovações"""
...
class DocumentService:
    ...
    def _make_json_serializable(obj):
        """Converte objetos para tipos JSON-serializáveis (como datetime)"""
    ...
    def extract_file_data(file):
        """Extrai dados do arquivo em formato JSON de forma segura sem salvar localmente"""
        try:
    ...
    def process_uploaded_file(file, user_id, document_id=None, document_name=None):
        ...
        - Cria um novo documento (se document_id for None e document_name for fornecido)
        - Ou cria uma nova versão para um documento existente
        - Extrai e armazena os dados
        - Compara a nova versão com a última versão aprovada
        """
        try:
            ...
            if existing_version:
                ...
                return existing_version, {
                    'differences_count': comparison.differences_count if comparison else 0,
    ...
    def approve_version(version_id, user_id, comment=''):
        """Aprova uma versão de documento e bloqueia para alterações"""
        try:
    ...
    def reject_version(version_id, user_id, comment=''):
        """Rejeita uma versão de documento e bloqueia para alterações"""
        try:
            version = DocumentVersion.query.get(version_id)
            if not version:
                raise ValueError("Versão de documento não encontrada")
    ...
    def get_documents_by_user(user_id):
        """Retorna todos os documentos criados por um usuário"""
    ...
    def format_version(version):
        """Formata metadados da versão para exibição no template/JSON"""
        ...
        return {
            ...
            'uploaded_at': version.created_at.strftime('%d/%m/%Y %H:%M:%S'),
            'approved_by': approver.name if approver else None,
            'approved_at': version.approved_at.strftime('%d/%m/%Y %H:%M:%S') if version.approved_at else None,
            'rejected_at': version.rejected_at.strftime('%d/%m/%Y %H:%M:%S') if version.rejected_at else None,
