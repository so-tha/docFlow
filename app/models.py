...
class Document(db.Model):
    """Documento - Container lógico para as versões"""
...
class Comparison(db.Model):
    """Modelo de Comparação - Compara uma versão com outra anterior do mesmo documento"""
    ...
    differences_data = db.Column(db.JSON)  # {differences: [{sheet, row, col, old_value, new_value}]}
...
class AuditLog(db.Model):
    ...
    action = db.Column(db.String(50), nullable=False, index=True)  # upload, approve, reject, login, logout, view_comparison
    
    entity_type = db.Column(db.String(50), index=True)  # document, document_version, comparison, user
